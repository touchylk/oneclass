...
class YOLONet(object):
    ...
    def __init__(self, is_training=True):
        ...
        if self.num_class == 1:
            ...
            self.one_class = 0
        if self.one_class:
            self.output_size=(self.cell_size * self.cell_size) * (self.boxes_per_cell * 5)
        else:
        ...
        if is_training:
            self.labels = tf.placeholder(tf.float32, [None, self.cell_size, self.cell_size, 5 ])
    ...
    def calc_iou(self, boxes1, boxes2, scope='iou'):
        """calculate ious
        Args:
          boxes1: (batch_size,7,7,2,4) 4-D tensor [CELL_SIZE, CELL_SIZE, BOXES_PER_CELL, 4]  ====> (x_center, y_center, w, h)
          boxes2: (batch_size,7,7,2,4) ()1-D tensor [CELL_SIZE, CELL_SIZE, BOXES_PER_CELL, 4] ===> (x_center, y_center, w, h)
        Return:
          iou: (b,7,7,2) 3-D tensor [CELL_SIZE, CELL_SIZE, BOXES_PER_CELL]
    ...
    def loss_layer(self, predicts, labels, scope='loss_layer'):
        ...
        :param predicts: 网络的输出,(batch_size,numoupt)
        :param labels: 标签,(batch_size,7,7,5)
        labels: 为(batch_size,7,7,5),7,7为格子,目标储存在对应的格子的维数上面
        ...
        with tf.variable_scope(scope):
            ...
            # predicts 0:self.boundary2是IOU,self.boundary2:是坐标
            ...
            # predict_boxes为(batch_size,7,7,2,4)

            # labels (batch_size,7,7,5)
            response = tf.reshape(labels[:, :, :, 0], [self.batch_size, self.cell_size, self.cell_size, 1])
            # response (b,7,7,1)
            boxes = tf.reshape(labels[:, :, :, 1:5], [self.batch_size, self.cell_size, self.cell_size, 1, 4])
            # boxes (batch_size,7,7,1,4)

            boxes = tf.tile(boxes, [1, 1, 1, self.boxes_per_cell, 1]) / self.image_size
            # boxes (batch_size,7,7,2,4)
            # classes = labels[:, :, :, 5:]
            ...
            # predict_boxes_tran (7,7,2,4,batch_size)

            predict_boxes_tran = tf.transpose(predict_boxes_tran, [1, 2, 3, 4, 0])
            # predict_boxes_tran (batch_size,7,7,2,4)
            ...
            # tf.cast()返回为cal_iou()的形状(b,7,7,2),response为(b,7,7,1)
            # object_mask为(b,7,7,2)
